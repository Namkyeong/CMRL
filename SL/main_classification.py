    ...
    with torch.autograd.detect_anomaly():
        for epoch in range(args.epochs):
            ...
            for index, batch_pair in enumerate(batches):
                ...
                neg_label = torch.ones_like(neg_prediction, dtype=torch.float).to(device) / 2
                neg_loss = F.kl_div(neg_prediction, neg_label, reduction = "sum")
                loss += args.lam1 * neg_loss

                rand_loss = F.binary_cross_entropy(rand_prediction, data['target'], reduction='sum')
