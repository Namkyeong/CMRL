...
def create_batch_mask(data, device):
    batch0 = data["g1"].batch.reshape(1, -1)
    index0 = torch.cat([batch0, torch.tensor(range(batch0.shape[1])).reshape(1, -1).to(device)])
    mask0 = torch.sparse_coo_tensor(index0, torch.ones(index0.shape[1]).to(device), size = (batch0.max() + 1, batch0.shape[1]))

    batch1 = data['g2'].batch.reshape(1, -1)
    index1 = torch.cat([batch1, torch.tensor(range(batch1.shape[1])).reshape(1, -1).to(device)])
    mask1 = torch.sparse_coo_tensor(index1, torch.ones(index1.shape[1]).to(device), size = (batch1.max() + 1, batch1.shape[1]))
