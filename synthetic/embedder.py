...
from report import config2string
from utils import create_batch_mask, get_roc_score, Dataclass
...
class embedder:
    ...
    def evaluate(self, epoch, final = False):
        ...
        for bc, samples in enumerate(self.val_loader):
            ...
            output = self.model([samples[0].to(self.device), samples[1].to(self.device), masks[0].to(self.device), masks[1].to(self.device)], test = True)
