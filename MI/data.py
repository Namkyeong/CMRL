...
def get_graph_from_smile(molecule_smile, idx):
    ...
    for i in range(molecule.GetNumAtoms()):
        ...
        # Use get_atom_features_mnsol if you are building MNSol dataset
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(f'data/raw/chr_abs.csv', sep=",")
    # Chr : (Chromophore, Solvent, Absorption max (nm)/ Emission max (nm)/ Lifetime (ns))
    processed_data, dataset = build_dataset(df, "Chromophore", "Solvent", "Emission max (nm)")
